...
def _render_deleted_archive_restore_expense(actor: Actor) -> None:
    ...
    if selected_row and st.sidebar.button("一鍵還原支出紀錄", key="expense_archive_restore_btn", use_container_width=True):
        ...
        st.rerun()
    if selected_row and st.sidebar.button("永久刪除此備援", key="expense_archive_purge_btn", type="secondary", use_container_width=True):
        archive_id = str(selected_row.get("archive_id", "")).strip()
        try:
            _purge_deleted_archive_local(archive_id)
        except Exception:
            pass
        try:
            get_api()._post("deleted_archive_purge", actor=actor, payload={"archive_id": archive_id, "system": "expense"})
        except Exception:
            pass
        st.sidebar.success("已刪除此備援紀錄。")
        st.rerun()




def _purge_deleted_archive_local(archive_id: str) -> bool:
    archive_id = str(archive_id or "").strip()
    if not archive_id:
        return False
    rows = load_json_cache("deleted_archive.json", default=[])
    ...
    for row in rows:
        if not isinstance(row, dict):
            new_rows.append(row)
            continue
        current_id = str(row.get("archive_id") or "").strip()
        if current_id == archive_id:
            removed = True
            continue
        new_rows.append(row)
    if removed:
        save_json_cache("deleted_archive.json", new_rows)
    return removed


def _purge_expense_record_from_local_views(actor: Actor, record_id: str) -> None:
    record_id = str(record_id or "").strip()
    if not record_id:
        return
    try:
        remove_local_expense_draft(actor.email, record_id, mark_deleted=False)
    except Exception:
        pass
    try:
        removed = 0
        for item in load_pending_sync(actor.email) or []:
            ...
            if event_id:
                removed += remove_pending_sync_item(actor.email, event_id=event_id, system_type="expense")
        if removed:
            st.session_state["expense_pending_removed"] = removed
    except Exception:
        pass
    try:
        _invalidate_expense_master(actor)
...
def load_record_into_form(record: Dict[str, Any], actor: Actor, grouped_options: Dict[str, List[str]]) -> None:
    ...
    form_data = {
        ...
        "owner_name": record.get("owner_name") or record.get("created_by_name") or actor.name,
...
def render_form_page(grouped_options: Dict[str, List[str]], defaults: Dict[str, Any]) -> None:
    ...
    with c2:
        st.text_input("填表人", value=str(form_data.get("owner_name") or actor.name or ""), disabled=True, key="exp_created_by_name_display")
    with c3:
...
def _owner_text(rec: Dict[str, Any]) -> str:
    return str(rec.get("owner_name") or rec.get("created_by_name") or rec.get("employee_name") or "").strip()
